...
import { DollarSign, Building2, Clock, CheckCircle, AlertCircle, TrendingUp, Users, Calendar, Search, Filter } from 'lucide-react';
...
const RentDistribution: React.FC = () => {
  ...
  const handleDepositRent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`Rent deposited successfully for Property #${formData.propertyId}`);
  ...
  const filteredPools = rentPools.filter(pool => {
    const matchesSearch = pool.address.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         pool.propertyId.includes(searchTerm);
    const matchesFilter = filterStatus === 'all' || pool.upkeepStatus === filterStatus;
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Rent Distribution</h1>
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <ul className="text-sm text-blue-700 dark:text-blue-300 mt-1 space-y-1">
                    <li>• Ensure you have approved MockUSDC for the RentDistributionVault contract</li>
                    <li>• Rent will be distributed proportionally to all token holders</li>
                    <li>• Distribution occurs automatically via Chainlink Automation</li>
                  </ul>
                </div>
              </div>
            </div>
      ...
      {!isOwner && (
        ...
        >
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="font-medium text-yellow-800 dark:text-yellow-400">Owner Access Required</h3>
              <p className="text-sm text-yellow-700 dark:text-yellow-300 mt-1">
                Only the contract owner can deposit rent. You can view rent pool status below.
              </p>
            </div>
          </div>
        </motion.div>
          ...
          <table className="w-full">
            <thead>
              <tr className="text-left border-b border-gray-200 dark:border-gray-700">
                ...
                <th className="pb-3 text-sm font-medium text-gray-600 dark:text-gray-400">Pool Balance</th>
                <th className="pb-3 text-sm font-medium text-gray-600 dark:text-gray-400">Total Distributed</th>
                <th className="pb-3 text-sm font-medium text-gray-600 dark:text-gray-400">Token Holders</th>
                <th className="pb-3 text-sm font-medium text-gray-600 dark:text-gray-400">Status</th>
                <th className="pb-3 text-sm font-medium text-gray-600 dark:text-gray-400">Next Distribution</th>
              </tr>
            </thead>
                ...
                >
                  ...
                  <td className="py-4">
                    <span className={`px-3 py-1 rounded-full text-xs font-medium flex items-center space-x-1 w-fit ${
                      ...
                      <span>{pool.upkeepStatus === 'ready' ? 'Ready' : 'Pending'}</span>
                    </span>
                  </td>
                  <td className="py-4">
                    <div className="flex items-center space-x-2">
